...
impl App {
    ...
    /// Run the app, this opens the TUI and handles key events and such.
        ...
        loop {
            ...
            if event::poll(std::time::Duration::from_millis(100))? {
                ...
                let tui_update = match action.clone() {
                    Action::EditNotes(project) => self.edit_notes(project),
                    Action::Quit => {
                        self.cleanup();
                        return Ok(());
                    }
    ...
    fn edit_notes(&mut self, project: Fireplace) -> Action {
        if let Ok(updated_notes) = open_editor(
            project.notes.clone(),
            Some(".md".to_string()),
            &mut self.terminal,
        ) {
            let mut new_project = project.clone();
            new_project.notes = updated_notes.clone();
